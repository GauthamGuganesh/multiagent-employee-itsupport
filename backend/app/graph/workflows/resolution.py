"""Resolution workflow: validate → persist ticket record → notify → close."""
...
def _compose_response(state: SupportState) -> str:
    if state.final_response:
        return state.final_response
    for result in reversed(state.specialist_results):
        if result.outcome == "resolved" and result.resolution_summary:
...
async def resolution_node(state: SupportState) -> dict:
    response = _compose_response(state)
    ...
    if ticket_number and ticket_number not in response:
        response = f"{response}\n\n(Tracked as ticket {ticket_number}, now resolved.)"
